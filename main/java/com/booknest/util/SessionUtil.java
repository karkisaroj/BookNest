...
/**
 * Utility class for managing HTTP sessions in the application. Provides methods
 * for setting, getting, and removing session attributes, checking login status,
...
public class SessionUtil {
	...
	/**
	 * Gets an attribute from the session, returning null if not found. Does not
	 * create a session if one doesn't exist. Provides type safety via casting.
	 *
	 * @param req  the HttpServletRequest
	 * @param key  the attribute key
	...
	/**
	 * Checks if a user session exists and contains a specific attribute (e.g.,
	 * username). Useful for checking login status without retrieving the actual
	 * value.
	 *
	 * @param req              the HttpServletRequest
	 * @param userAttributeKey The session key that indicates a dlogged-in user
	 *                         (e.g., "userName")
	 * @return true if the session exists and the user attribute is present, false
	 *         otherwise.
	 */
